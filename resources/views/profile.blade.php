        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    ...
                    @foreach ($data as $d)
                        <div class="card-header">{{ __($d->name) }}
                            <form method="post" action="{{ route('accounts.destroy', $d->id) }}"
                                onclick="return confirm('Are you sure you want to delete this item')" class="float-end">
                                @method('delete')
                                @csrf
                        ...
                        <div class="card-body ">
                            <form method="post" action="{{ route('accounts.update', $d->id) }}">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-6">
                                        <input type="text" class="form-control @error('email') is-invalid @enderror"
                                            id="email" name="email" placeholder="Email" required autocomplete="email"
                                            value="{{ $d->email }}">
                                        @error('email')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                ...
                                <div class="row mb-3">
                                    <label for="baranggay"
                                        class="col-md-4 col-form-label text-md-end">{{ __('Baranggay') }}</label>
